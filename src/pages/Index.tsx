...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-foreground transition-colors duration-300">
      ...
      {/* ── Main content ── */}
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-5 print:hidden">
        {/* SEO H1 */}
        ...
        <div className="rounded-2xl overflow-hidden shadow-md border border-slate-200 dark:border-slate-700 flex flex-col">
          ...
          <div className="bg-gradient-to-r from-[#3b3fc7] to-[#6d5ee0] flex flex-col md:flex-row items-stretch divide-y md:divide-y-0 md:divide-x divide-white/10">
            <div className="flex items-center gap-3 sm:gap-6 px-3 sm:px-6 py-2.5 sm:py-5 flex-[2]">
              <div className="shrink-0">
                <p className="text-[7px] sm:text-[9px] font-bold uppercase tracking-widest text-white/50 mb-0.5">Current GPA</p>
                <p className="text-2xl sm:text-5xl font-black tabular-nums leading-none text-white">{stats.classGPA}</p>
              </div>
              <div className="w-px h-8 sm:h-12 bg-white/15 hidden sm:block" />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5 sm:gap-3 flex-wrap">
                  <div>
                    <p className="text-[7px] sm:text-[9px] font-bold uppercase tracking-widest text-white/50">Awarded Class</p>
                    <p className="text-[11px] sm:text-sm font-bold text-white mt-0.5 flex items-center gap-1"><span>🥇</span>{stats.degreeClass}</p>
                  </div>
                  <span className="inline-flex items-center gap-1 bg-white/15 rounded-full px-1.5 sm:px-2.5 py-0.5">
                    <span className="text-[9px] sm:text-[11px] font-semibold text-white">{stats.degreeEligible ? "Eligible" : "Not classified"}</span>
                    <Info className="h-2 w-2 sm:h-2.5 sm:w-2.5 text-white/60" />
                  </span>
                </div>
                <div className="mt-1.5 sm:mt-2 flex items-center gap-1.5 sm:gap-2">
                  <div className="flex-1 h-0.5 sm:h-1 bg-white/20 rounded-full overflow-hidden max-w-[180px]">
                    <div className="h-full bg-white rounded-full transition-all duration-700" style={{ width: `${stats.completionPercent}%` }} />
                  </div>
                  <span className="text-[9px] sm:text-[11px] text-white/60 tabular-nums">{stats.completionPercent}%</span>
                </div>
              </div>
            </div>
            <div className="flex-[3] px-3 sm:px-6 py-2.5 sm:py-5">
              <p className="text-[7px] sm:text-[9px] font-bold uppercase tracking-widest text-white/50 mb-1.5 sm:mb-3">GPA Scale (UCSC)</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-x-2 sm:gap-x-4 gap-y-1 sm:gap-y-2">
                {GPA_SCALE_BANDS.map((band) => (
                  <div key={band.range}>
                    <p className="text-[11px] sm:text-sm font-bold text-white tabular-nums leading-tight">{band.range}</p>
                    <p className="text-[9px] sm:text-[11px] text-white/55 mt-0.5 leading-tight">{band.label}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-slate-100 dark:bg-slate-800/80 px-3 sm:px-6 py-2.5 sm:py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-5 border-t border-slate-200 dark:border-slate-700/50">
            <div className="flex items-center gap-2.5 sm:gap-4">
              <div className={`w-8 h-8 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-white dark:bg-slate-700 shadow-sm flex items-center justify-center text-base sm:text-xl`}>
                {activeYearMeta.emoji}
              </div>
              <div className="text-slate-900 dark:text-white">
                <p className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Currently Viewing</p>
                <p className="text-sm sm:text-lg font-black mt-0.5">{activeYearData.title}</p>
                <p className="text-[9px] sm:text-[11px] text-slate-500 mt-0.5 hidden sm:block">Grades are saved automatically when you switch years.</p>
              </div>
            </div>
              ...
              {YEAR_META.map((meta) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl text-[11px] sm:text-sm font-bold transition-all active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/50 min-h-[34px] sm:min-h-[40px] ${
        ...
        {yearRuleResult && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            {/* Header */}
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 dark:border-slate-700/60">
              <div>
                <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  Year Progress Check
                </p>
                <h2 className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-100 mt-0.5">
            ...
            {/* GPA grid */}
            <div className="grid grid-cols-2 gap-3 sm:gap-4 px-4 sm:px-6 py-3 sm:py-4">
              <div className="rounded-xl bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700 px-3 sm:px-4 py-2.5 sm:py-3">
                <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-slate-400">Overall GPA</p>
                <p className="text-xl sm:text-2xl font-black tabular-nums text-slate-900 dark:text-white mt-1">
                  {yearRuleResult.overallGpa}
                </p>
              </div>
              <div className="rounded-xl bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700 px-3 sm:px-4 py-2.5 sm:py-3">
                <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-slate-400">Class GPA</p>
                <p className="text-xl sm:text-2xl font-black tabular-nums text-slate-900 dark:text-white mt-1">
            ...
            <div className="px-4 sm:px-6 pb-4 sm:pb-5 space-y-2">
              {yearRuleResult.checks.map((check) => (
                <div
                  key={check.id}
                  className={`flex items-start justify-between gap-2 sm:gap-3 rounded-xl border px-3 sm:px-3.5 py-2 sm:py-2.5 ${
                    check.satisfied
                ...
                >
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-slate-800 dark:text-slate-100">{check.label}</p>
                    {check.detail && (
                      <p className="mt-0.5 text-[11px] sm:text-xs text-slate-500">{check.detail}</p>
                    )}
                  </div>
                  <span
              ...
              {/* Disclaimer */}
              <div className="rounded-xl border border-amber-200 dark:border-amber-800/40 bg-amber-50 dark:bg-amber-950/20 px-3 sm:px-3.5 py-2 sm:py-2.5 mt-3">
                <p className="text-[11px] sm:text-xs font-medium text-amber-700 dark:text-amber-300">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 pb-2">
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed min-h-[40px]"
            >
              ← Previous Year
            </button>
            <button
          ...
          {activeYear === 3 && (
            <div className="flex flex-col items-stretch sm:items-end gap-1">
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs sm:text-sm font-bold shadow-md hover:shadow-lg hover:shadow-violet-500/25 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none min-h-[44px]"
              >
      ...
      {showFinalPopup && (
        ...
        >
          <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 mx-4">
            {/* Header */}
            <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-4 sm:px-6 pt-5 sm:pt-6 pb-4 sm:pb-5 text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="text-base sm:text-lg font-black text-white">🎉 Congratulations!</h3>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2 sm:gap-3 mt-4 sm:mt-5">
                <div className="bg-white/15 rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3">
                  <p className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest text-white/60">Overall GPA</p>
                  <p className="text-2xl sm:text-3xl font-black tabular-nums text-white mt-1">{stats.overallGPA}</p>
                </div>
                <div className="bg-white/15 rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3">
                  <p className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest text-white/60">Class GPA</p>
                  <p className="text-2xl sm:text-3xl font-black tabular-nums text-white mt-1">{stats.classGPA}</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-3 sm:space-y-4">
              {/* Awarded class */}
              <div className="rounded-2xl border border-violet-200 dark:border-violet-800/50 bg-violet-50 dark:bg-violet-950/20 px-3 sm:px-4 py-2.5 sm:py-3">
                <p className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest text-violet-500">🏅 Awarded Class</p>
                <p className="text-base sm:text-lg font-black text-slate-900 dark:text-white mt-1">{stats.degreeClass}</p>
              </div>

              {/* Year GPAs */}
              <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                {[1, 2, 3].map((y) => (
                  <div key={y} className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/40 px-2 sm:px-3 py-2 sm:py-2.5 text-center">
                    <p className="text-base sm:text-lg">{YEAR_META[y - 1].emoji}</p>
                    <p className="text-[9px] sm:text-[10px] font-semibold text-slate-400 mt-0.5">Year {y}</p>
                    <p className="text-xs sm:text-sm font-black tabular-nums text-slate-900 dark:text-white">{stats.yearResults[y].gpa}</p>
                  </div>
                ))}
              </div>
              ...
              {/* Eligibility */}
              <div
                className={`flex items-start gap-2 sm:gap-2.5 rounded-xl border px-3 sm:px-3.5 py-2 sm:py-2.5 ${
            ...
            <div className="px-4 sm:px-6 pb-4 sm:pb-5 flex gap-2 justify-end">
              <button
                onClick={handleExport}
                className="px-3 sm:px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all min-h-[40px]"
              >
                Export PDF
              </button>
              <button
                onClick={() => setShowFinalPopup(false)}
                className="px-4 sm:px-5 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs sm:text-sm font-bold hover:shadow-md hover:shadow-violet-500/25 transition-all active:scale-95 min-h-[40px]"
              >
                Close
              </button>
            </div>
      ...
      {/* ── Footer ────────────────────────────────────────── */}
      <footer className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6 no-print">
        <div className="h-px bg-slate-200 dark:bg-slate-700 mb-4 sm:mb-5" />
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-3">
          <p className="text-[11px] sm:text-xs text-slate-400">UCSC BIT GPA Calculator · Academic Year 2024</p>
          <a
